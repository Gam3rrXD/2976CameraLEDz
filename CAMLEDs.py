...
# LED control
@app.route('/leds', methods=['GET']) #Setting Contact URL
def leds(): #Defining LED function
    # Collecting ARGS passed using url (color=red&effect=strobe&comment=redstrobeeffect)
    color = request.args.get('color')
    effect = request.args.get('effect')
    comment = request.args.get('comment')

    return "Received color: {}, effect: {}, comment: {}".format(color, effect, comment) # outputting recived commands for now, LED implementation WIP
